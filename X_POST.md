# X participation post (#OKXAI)

Rules: introduce ASP, use case, **demo ≤ 90 seconds**, hashtag **#OKXAI**.

**Studio:** https://pixelbrief.vercel.app  
**API:** https://pixelbrief.vercel.app/api  
**Health:** https://pixelbrief.vercel.app/health

## 90-second demo script

1. (0–10s) Show okx.ai listing card: “PixelBrief — Art creation”
2. (10–35s) Agent calls `/v1/brand-kit?name=...` → payment toast → JSON returns
3. (35–65s) Paste SVG into browser / VS Code preview → show logo + palette swatches
4. (65–90s) Show social captions + CTA: “Hire PixelBrief on OKX.AI”

Record landscape 1080p. No music required. Hard cut, readable UI zoom.

## Tweet / thread

**Post 1 (main — must include video + #OKXAI):**
```text
PixelBrief is live on OKX.AI 🎨

One prompt → full brand kit:
• logo SVG
• palette + CSS vars
• font pairing
• 3 social posts
• thumbnail brief

Built as an A2MCP ASP — agents pay per call on X Layer.

Demo ↓
#OKXAI
```

**Post 2:**
```text
Why I built it:
Most agents can write copy.
Almost none can ship a visual system in one paid call.

PixelBrief turns a name + mood into something you can actually brand with.
```

**Post 3:**
```text
Try it on OKX.AI → Art creation → PixelBrief
Agent ID: 5421
Endpoint: https://pixelbrief.vercel.app/v1/brand-kit
Studio: https://pixelbrief.vercel.app

Hackathon: OKX.AI Genesis
```

## Social Buzz tips
- Post when your audience is awake; pin the thread
- Reply to every comment in the first 6 hours
- Quote-tweet with a still of the generated logo
- Share in HackQuest / OKX builder chats with the same demo clip
- Do **not** buy fake engagement; OKX judges quality + reach
