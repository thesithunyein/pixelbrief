...
const REQUIRE_PAYMENT = (process.env.REQUIRE_PAYMENT ?? "true").toLowerCase() !== "false";
...
async function buildApp() {
  ...
  if (REQUIRE_PAYMENT) {
    if (!PAY_TO || !process.env.OKX_API_KEY || !process.env.OKX_SECRET_KEY || !process.env.OKX_PASSPHRASE) {
      throw new Error(
        "REQUIRE_PAYMENT=true needs PAY_TO_ADDRESS, OKX_API_KEY, OKX_SECRET_KEY, OKX_PASSPHRASE. Set REQUIRE_PAYMENT=false for local free mode.",
      );
    }
...
app.listen(PORT, () => {
  console.log(`[PixelBrief] listening on :${PORT}`);
  console.log(`[PixelBrief] payment=${REQUIRE_PAYMENT ? "x402" : "off"} network=${NETWORK}`);
  console.log(`[PixelBrief] public=${PUBLIC_BASE_URL}`);
});
